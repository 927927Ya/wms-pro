...
const PalletPickingPage = () => {
  ...
  const columns = [
    ...
    { title: '出库明细ID', dataIndex: 'outboundDetailsId', key: 'outboundDetailsId' },
    ...
    { title: '状态', dataIndex: 'taskStatus', key: 'taskStatus', render: (v) => <Tag>{taskStatusMap[v] || v}</Tag> },
    {
      ...
      render: (_, r) => (
        r.taskStatus !== 2 && hasAnyPermission(['outbound:manage', 'outbound:pick']) && (
          <Button type="primary" size="small" onClick={async () => { await completePickingTask(r.id, 0); message.success('拣货完成'); refresh(); }}>完成拣货</Button>
        )
      ),
    },
  ];
...
export default PalletPickingPage;
